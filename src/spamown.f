...
c     colmn -- logical vector of length ncol
      ...
      implicit none
      integer(8)  nrow,ncol,nnz,nz,inz,
     &       ja(nnz),ia(nrow+1),jao(nz),iao(nrow+1)
      logical(8) colmn(ncol)
      integer(8) i,j,k
      inz = 0
      iao(1) = 1
      do i = 1,nrow
      ...
      integer(8) ja(*), ia(nrow+1), iw(nrow)
c-----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
      ...
      implicit none
      integer(8) nrow,ncol,ja(*),ia(nrow+1)
      double precision a(*), b(nrow,*)
...
c-----------------------------------------------------------------------
      ...
      integer(8) nrow, ncol, ia(*), ja(*), ic(*), jc(*), cmax
c
...
c     Local
      ...
      integer(8) i,k, len, ib(nrow+1), jb(nrow)
...
c-----------------------------------------------------------------------
      ...
      implicit none
      integer(8) nrow,ncol,nzmax
      integer(8) ja(*),jb(*),jc(*),ia(*),ib(*),ic(*)
...
c     local variables
      integer(8) i,j1,j2,ka,kb,kc,kamax,kbmax
...
c-----------------------------------------------------------------------
      ...
      implicit none
      integer(8) i,j,next
      integer(8) nrow,ncol,ndns,ia(*),ja(*)
...
c----------------------------------------------------------------------- 
      implicit none
      integer(8) nrow,nnz,ir(*),jc(*),jao(*),iao(*)
      integer(8) k,k0,j,i,iad
c-----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
      ...
      integer(8) nrw,rw(*),  ncl, cl(*)
      integer(8) bnz, ia(*),ja(*), ib(*),jb(*)
      double precision a(*),b(*)
c
c     local variables.
c
      integer(8) irw, jcl, jja
c
c      write(*,*) cl(1),cl(2)
      bnz = 1
      ib(1) = 1
...
c----------------------------------------------------------------------- 
      ...
      integer(8) i, ia(*), iadd, j, ja(*)
      double precision a(*),elem
c
c     local variables.
c
      integer(8) ibeg, iend, imid
c
c     initialization 
c
      iadd = 0 
...
c----------------------------------------------------------------------- 
      ...
      integer(8) nir,ir(nir),jr(nir),ja(*),ia(*),alliadd(nir)
...
c---------------------------------------------------------------------
      ...
      integer(8) ia(nrow+1),ja(*),jc(*),ic(nrow+1),jmask(*)
      integer(8) imask(nrow+1), nzmax, ierr
      logical(8) iw(ncol)
c-----------------------------------------------------------------------
c further used variables
      integer(8) len, k, k1, k2, ii, j
c-----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
     ...
     *     nzmax,ierr)
      implicit none
      integer(8) nrow, ncol, nzmax, ierr
      real(8) a(*), b(*), c(*)
      integer(8) ja(*),jb(*),jc(*),ia(nrow+1),ib(nrow+1),ic(nrow+1)
c-----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
      integer(8) i, ka, kb, kc, kamax, kbmax, j1, j2
      ierr = 0
      kc = 1
      ic(1) = kc 
c
...
c-----------------------------------------------------------------------
     ...
     *     c,jc,ic,nzmax,iw,aw,ierr)
      implicit none
      integer(8) nrow, ncol, job, nzmax, ierr
...
c-----------------------------------------------------------------------
      logical(8) values
      integer(8) len, j, ii, ka, kb, k, jpos, jcol
      values = (job .ne. 0) 
      ierr = 0
      len = 0
      ic(1) = 1 
...
c-----------------------------------------------------------------------
      logical(8) values
      integer(8) len, kb, ka, k, jpos, jcol, j, ii
      values = (job .ne. 0) 
      ierr = 0
      len = 0
      ic(1) = 1 
...
c     cholcja (integer)        ja for cholesky factor
      ...
      integer(8) nrow,nsuper
      integer(8) xsuper(nrow),lindx(*),xlindx(nrow+1),xlnz(nrow+1)
      integer(8) cholcja(*)
      ...
      implicit none
      integer(8) n,m,ia(n+1),iao(m+1),ja(*),jao(*)
      double precision  a(*),ao(*)


      integer(8) i,j,k,next
c-----------------------------------------------------------------------
c     Transposition
...
c-----------------------------------------------------------------------
      ...
      integer(8) bnrow, bncol,k
      integer(8) ia(*),ja(*),ib(*),jb(*)
...
c Currently not used...
      ...
      integer(8) nrow, ncol,k
      integer(8) ia(*),ja(*)

      integer(8)   i, j, jj, itmp
