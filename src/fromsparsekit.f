...
c-----------------------------------------------------------------------
     ...
     *     nzmax,ierr)
      ...
      integer(8) nzmax, ierr
c-----------------------------------------------------------------------
c further used vaeriables
      integer(8) i, j1, j2, ka, kamax, kb, kbmax, kc
...
c
      ...
      implicit none
      integer(8) job,i1,i2,j1,j2,nr,nc,ia(*),ja(*),jao(*),iao(*)
      real(8) a(*),ao(*)
c-----------------------------------------------------------------------
c further used variables
      integer(8) i, ii, j, k, k1, k2, klen
c-----------------------------------------------------------------------
...
c
      ...
      implicit none
      integer(8) nrow, ncol, ncolb
      integer(8) ja(*),jb(*),ia(nrow+1),ib(ncol+1)
      integer(8) ndegr(nrow),iw(ncolb)
      integer(8) nnz
c-----------------------------------------------------------------------
c further used variables
      integer(8) ii, j, jc, jr, k, last, ldg
c-----------------------------------------------------------------------
...
c
     ...
     *                  c,jc,ic,nzmax,iw,ierr)
      implicit none
      real(8) a(*), b(*), c(*)
      integer(8) nrow, ncol, job
      integer(8) ja(*),jb(*),jc(*),ia(nrow+1),ib(*),ic(*),iw(ncol)
      integer(8) nzmax, ierr
c-----------------------------------------------------------------------
c other used variables
      integer(8) len, k, ka, kb, jpos, jcol, ii, jj, j
c-----------------------------------------------------------------------
...
c-
      ...
      integer(8) n
      real(8) a(*),ao(*),wk(n)
      integer(8) ia(n+1),ja(*),jao(*),iwk(n+1)
c-----------------------------------------------------------------------
c other used variables
      integer(8) k, j, iptr, ii, icount, i
c-----------------------------------------------------------------------
...
c
      ...
      implicit none
      real(8) diag(*),a(*)
      integer(8) nrow, ncol, job, len, ioff, ia(*), ja(*), idiag(*)
c-----------------------------------------------------------------------
c further used variables
      integer(8) izero
c-----------------------------------------------------------------------
...
c----------------------------------------------------------------------c
c     local variables
      integer(8) istart, max, iend, i, kold, k, kdiag, ko
c
      izero = 0
