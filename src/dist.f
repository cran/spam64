...
c     p power for minkowski
     ...
     &    eta, colindices, rowpointers, entries, nnz, iflag)
      ...
      integer(8) ncol,nrowx, nrowy, nnz, method,  part,  iflag
      integer(8) colindices(nnz), rowpointers(nrowx+1)
     ...
     &    eta, colindices, rowpointers, entries, nnz, iflag)
      ...
      integer(8) ncol,xnrow, ynrow, nnz,  part,iflag
      integer(8) colindices(nnz),rowpointers(xnrow+1)
      double precision p,x(xnrow,ncol), y(ynrow,ncol)
      double precision eta, entries(nnz)
      
c     local variables
      integer(8) jja, i,j,k, jfrom, jto
...
c     in case nnz was too small, recall line to get a better estimate
 ...
 20   continue
     ...
     &    eta, colindices, rowpointers, entries, nnz, iflag)
      ...
      integer(8) nx, ny, nnz, colindices(nnz),rowpointers(nx+1)
      integer(8) part, iflag
      double precision x(nx,2), y(ny,2), p, eta, entries(nnz)
      
c     local variables
      logical(8) equi
      integer(8) jja, i,j, jfrom, jto
